...
%Varying W
function [W, dz, S, dzud, Zprev] = ADLSlabDesign3(Zin, ZL, dy, h, N, erHost, m, fr0, th, ph, tol, type, shift)
    %Initial setups
    omega = 2.*pi.*fr0;
    c = 3e8;
    ...
    W_iter = 0.008*lam:0.0001*lam:0.06*lam;
...
%        disp(epsilon(1));
        if(abs(imag(Zprev))<10.000 & abs(Zin - real(Zprev))<tol*Zin) 
            W = ind;
            break;
        else
            continue;
